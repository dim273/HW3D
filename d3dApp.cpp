...
LRESULT D3DApp::MsgProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_SIZE:
        ...
        //当用户抓取调整信息框的大小时发送 WM_EXITSIZEMOVE
    case WM_ENTERSIZEMOVE:
        m_AppPaused = true;
        m_Resizing = true;
        m_Timer.Stop();
        return 0;
        //当用户释放信息框的大小时发送 WM_EXITSIZEMOVE
    ...
    case WM_MENUCHAR:
        ...
        ((MINMAXINFO*)wParam)->ptMaxTrackSize.y = 200;
        return 0;
...
bool D3DApp::InitDirect3D()
{
...
#if defined(BEBUG) || defined(_DEBUG)
    ...
    //可以禁止alt+enter全屏
    dxgiFactory1->MakeWindowAssociation(m_hMainWnd, DXGI_MWA_NO_WINDOW_CHANGES);
...
{
    //该代码计算每一秒的帧数，并计算每一帧渲染需要的时间，显示在窗口标题
    static int frameCnt = 0;
    static float timeElapsed = 0.0f;

    frameCnt++;

    if ((m_Timer.TotalTimer() - timeElapsed) >= 1.0f)
    {
