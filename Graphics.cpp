...
const char* Graphics::HrException::what() const noexcept
{
	...
	oss << GetType() << std::endl
		<< "[错误代码] 0x" << std::hex << std::uppercase << GetErrorCode()
		<< std::dec << "(" << (unsigned long)GetErrorCode() << ")" << std::endl
		<< "[错误信息]" << GetErrorString() << std::endl
		<< "[错误描述]" << GetErrorDescription() << std::endl;
	if (!info.empty())
	{
...
{
	return "Birddim Graphics Exception [设备被移除] (DXCI_ERROR_DEVICE_REMOVE)";
}
